...
useEffect(() => {
  const checkEmailAvailability = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:4001/Services/check-email-availability", {
        email: emailData.new
      });
  ...
  const fetchSuperAdminStatus = async (userId) => {
    try {
      const res = await axios.post(`http://localhost:4001/User/get-profile-data`);
  ...
  try {
    const response = await axios.post("http://localhost:4001/User/change-password", payload);
...
const handleChangeEmail = async () => {
  ...
  try {
    const response = await axios.post("http://localhost:4001/User/change-email", payload);
...
console.log("oldkey", oldKey);
  ...
  try {
    ...
    const uploadRes = await axios.post("http://localhost:4001/Services/upload-pdf", formData, {
      headers: {
    ...
    const changeRes = await axios.post("http://localhost:4001/User/change-profile-photo", {
    ...
    if (changeRes.data.status === "OK") {
      if (oldKey) {
       await axios.delete("http://localhost:4001/Services/delete-pdf", {
        data: { fileKey: oldKey },
      });
...
const handleRemoveImage = async () => {
  try {
    await axios.post("http://localhost:4001/User/remove-profile-photo", {
      id: profileData.userId
    });
...
const handleChangePhone = async () => {
  ...
  try {
    const response = await axios.post("http://localhost:4001/User/change-phone", payload);
...
const handleSaveChanges = async () => {
  ...
  try {
    const response = await axios.post(
      "http://localhost:4001/User/change-personal-detail",
      payload
    );
...
useEffect(() => {
  ...
  const delayDebounce = setTimeout(() => {
    axios
      .post("http://localhost:4001/Services/check-username-availability", {
